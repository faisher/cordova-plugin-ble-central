...
- (void)startConnectTimer:(CBPeripheral *)peripheral timeout:(NSTimeInterval)timeout;
- (void)cancelConnectTimer:(CBPeripheral *)peripheral;
...
- (void)pluginInitialize {
    ...
    connectTimers = [NSMutableDictionary new];
...
- (void)connect:(CDVInvokedUrlCommand *)command {
    ...
    if (peripheral) {
        ...
        [self startConnectTimer:peripheral timeout:20.0];
    } else {
...
- (void)disconnect:(CDVInvokedUrlCommand*)command {
    ...
    if (!peripheral) {
        ...
        [self cancelConnectTimer:peripheral];
...
- (BOOL)validateWriteMessage:(NSData*)message peripheral:(CBPeripheral*)peripheral type:(CBCharacteristicWriteType)type callbackId:(NSString*)callbackId {
    if (message == nil || peripheral == nil) {
        return NO;
    }

    NSUInteger maxLength = 0;
    if ([peripheral respondsToSelector:@selector(maximumWriteValueLengthForType:)]) {
        maxLength = [peripheral maximumWriteValueLengthForType:type];
    }

    if (maxLength > 0 && [message length] > maxLength) {
        NSString *errorMessage = [NSString stringWithFormat:@"Write exceeds max length (%lu > %lu)",
                                  (unsigned long)[message length], (unsigned long)maxLength];
        NSLog(@"%@", errorMessage);
        CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
...
- (void)write:(CDVInvokedUrlCommand*)command {
    ...
    if (context) {
        if (message != nil && [message isKindOfClass:[NSData class]]) {
            ...
            CBCharacteristic *characteristic = [context characteristic];
            if (![self validateWriteMessage:message peripheral:peripheral type:CBCharacteristicWriteWithResponse callbackId:command.callbackId]) {
                return;
            }
...
- (void)writeWithoutResponse:(CDVInvokedUrlCommand*)command {
    ...
    if (context) {
        ...
        if (message != nil && [message isKindOfClass:[NSData class]]) {
            ...
            CBCharacteristic *characteristic = [context characteristic];
            if (![self validateWriteMessage:message peripheral:peripheral type:CBCharacteristicWriteWithoutResponse callbackId:command.callbackId]) {
                return;
            }
...
- (void)onReset {
    stateCallbackId = nil;
    for (NSTimer *timer in [connectTimers allValues]) {
        [timer invalidate];
    }
    [connectTimers removeAllObjects];
}
...
- (void)startConnectTimer:(CBPeripheral *)peripheral timeout:(NSTimeInterval)timeout {
    ...
    NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:timeout
                                                   ...
                                                   userInfo:peripheral
                                                    repeats:NO];
    [connectTimers setObject:timer forKey:[peripheral uuidAsString]];
}

- (void)cancelConnectTimer:(CBPeripheral *)peripheral {
    NSString *key = [peripheral uuidAsString];
    NSTimer *timer = [connectTimers objectForKey:key];
    if (timer) {
        [timer invalidate];
        [connectTimers removeObjectForKey:key];
    }
}
...
- (void)connectTimerFired:(NSTimer *)timer {
    ...
    NSString *key = [peripheral uuidAsString];
    NSLog(@"Connection timeout for peripheral %@", key);

    [connectTimers removeObjectForKey:key];
    [manager cancelPeripheralConnection:peripheral];

    NSString *connectCallbackId = [connectCallbacks valueForKey:key];
    [connectCallbacks removeObjectForKey:key];
    [self cleanupOperationCallbacks:peripheral withResult:[CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Connection timeout"]];

    if (connectCallbackId) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:[peripheral asDictionary]];
        [dict setObject:@"Connection timeout" forKey:@"errorMessage"];
        [dict removeObjectForKey:@"rssi"];
        [dict removeObjectForKey:@"advertising"];
        [dict removeObjectForKey:@"services"];
